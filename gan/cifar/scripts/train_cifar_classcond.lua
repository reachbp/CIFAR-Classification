...
--
function get10Samples(model, if_tanh)
  local N = 10
  --local model_G = model.G
  --opt = model.opt
  classes = {'0','1'}
  opt.geometry = {3, opt.scale, opt.scale}
  opt.condDim = 10
  ...
  for n = 1,numclasses do
     cond_inputs[n][n] = 1
     classes[n] = cifar_classes[n]
  end

  local samples = model_G:forward({noise_inputs, cond_inputs})
  if if_tanh then
        samples = nn.HardTanh():cuda():forward(samples)
  end
...
while true do
  ...
  if opt.plot  and epoch and epoch % 1 == 0 then
    ...
    trainLogger:plot()
    testLogger:plot()
    local to_plot, classes = get10Samples(adversarial, true)
